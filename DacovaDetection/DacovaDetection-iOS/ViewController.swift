...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate,AVCapturePhotoCaptureDelegate {
    ...
    let colors:[UIColor] = {
        ...
        for _ in 0...10 {
            let color = UIColor(red: CGFloat.random(in: 0...1), green: CGFloat.random(in: 0.1...1), blue: CGFloat.random(in: 0.1...1), alpha: 1)
    ...
    lazy var yoloRequest:VNCoreMLRequest! = {
        do {
            // change weight file name
            let model = try model_20231110_222829_531804().model
    ...
    func drawRectsOnImage(_ detections: [Detection], _ pixelBuffer: CVPixelBuffer) -> UIImage? {
        ...
        for detection in detections {
            ...
            if let labelText = detection.label {
                ...
                let textRect  = CGRect(x: invertedBox.minX - size.width * 0.01, y: invertedBox.minY + size.width * 0.06, width: invertedBox.width, height: invertedBox.height)
                let textStyle = NSMutableParagraphStyle.default.mutableCopy() as! NSMutableParagraphStyle
                
                let textFontAttributes = [
                    NSAttributedString.Key.font: UIFont.systemFont(ofSize: textRect.width * 0.3, weight: .bold),
